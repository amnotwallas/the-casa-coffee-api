...
class ProductService:
    ...
    def get_total_count(self, category: Optional[str] = None, search: Optional[str] = None) -> int:
        return self.repository.count_products(category=category, search=search)
    ...
    def get_categories(self) -> List[dict]:
        products = self.repository.list_all_products(limit=1000)
        cats = {}
        for p in products:
            c = p.categoria
            if c:
    ...
    def get_featured_products(self) -> List[ProductSchema]:
        products = self.repository.list_all_products(limit=100)
